...
public class ContextPipelineImpl implements Pipeline {
    ...
    public <S> Publisher<GetResult<S>> get(Class<S> stateType) {
        return Mono.from(delegate.get(stateType)).contextWrite(ctx -> ctx.put(Constants.SESSION_ID_CONTEXT_KEY, sessionIdSupplier.get()));
    }

    @Override
    public <S, A> Publisher<SetResult<S>> set(Class<S> stateType, A action) {
        return Mono.from(delegate.set(stateType, action)).contextWrite(ctx -> ctx.put(Constants.SESSION_ID_CONTEXT_KEY, sessionIdSupplier.get()));
    }
}
